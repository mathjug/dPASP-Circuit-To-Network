...
def test_network_builder_no_duplicates(node_type, description, sdd_file, json_file, expected_counts, should_simplify):
    ...
    neural_network_root = NetworkBuilder(node_type[0], node_type[1]).build_network(sdd_file, json_file, should_simplify, False)
...
def test_network_builder_node_caching():
    ...
    nn1 = NetworkBuilder(RecursiveORNode, RecursiveANDNode).build_network(sdd_file, json_file, False, False)
    nn2 = NetworkBuilder(RecursiveORNode, RecursiveANDNode).build_network(sdd_file, json_file, False, False)
